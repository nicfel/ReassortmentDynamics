...
reassortment.data$superspreading = "random k"
reassortment.data[reassortment.data$run<=50, "superspreading"] = "k = Infinity"
reassortment.data[reassortment.data$run<=50, "superspreading"] = 1000000000

p=ggplot(reassortment.data[reassortment.data$method=="ne",], aes(x=transmission*(1+1/k), y=events, ymin=eventsl, ymax=eventsu, color=superspreading))+
  geom_point()+geom_errorbar()+scale_y_log10() + scale_x_log10() + 
  ylab("estimated constant c for Ne=c*I ")+ theme_minimal()+scale_color_OkabeIto()
plot(p)
ggsave("./../../Figures/Sir_neRalation.pdf", p, width = 6, height = 3)
