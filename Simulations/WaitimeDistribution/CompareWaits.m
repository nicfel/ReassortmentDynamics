...
for iteration = 501 : 1000
    ...
    f = fopen(['master/SIR_simulations_' num2str(iteration) '.xml']);
    while ~feof(f)
        line = fgetl(f);
        if contains(line, 'transmissionRate=')
            transmission_rate = str2double(regexp(line, 'transmissionRate="(\d+\.\d+)"', 'tokens', 'once'));
        elseif contains(line, 'populationSize=')
            pop_Size = str2double(regexp(line, 'populationSize="(\d+)"', 'tokens', 'once'));
        end
    end
    fclose(f);

    
    data = textscan(fopen(['master/SIR_simulations_' num2str(iteration) '.log']), '%s', 'delimiter', '\t', 'headerlines', 1);
    ...
    % for i = 1:100
        ...
        for j = 2:length(lins)
            ...
            rate_since_last_interval = rate_since_last_interval + sum(transmission.*diff(time_I(idx2:idx1))*no_lins(end));
            
            if strcmp(tmp{1}, '0')
                no_lins = [no_lins no_lins(end) + 1];
            elseif strcmp(tmp{1}, '1')
                no_lins = [no_lins no_lins(end) + 1];
