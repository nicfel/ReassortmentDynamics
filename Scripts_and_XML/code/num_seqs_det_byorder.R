...
setwd("~/Dropbox/HPAI/H5-reassortment/prev-reassortment/H5N1_detections/")
...
HPAI_detections_date <- wildbird_count_date_wide %>%
  full_join(domes_count_date, by = "date_det") %>%
  full_join(mammals_count_date, by = "date_det") %>%
  full_join(cattle_count_date, by = "date_det") %>%
  mutate(across(everything(), ~replace_na(., 0))) %>%
  mutate(galliformes = galliformes + domestic_count) %>%
  mutate(nonhuman_mammal = mammal_count + cattle_count) %>%
  select(-domestic_count, -mammal_count, -cattle_count) %>%
  rowwise() %>%
  ungroup()

write.csv(HPAI_detections_date, file = "HPAI_detections_date.csv", row.names = FALSE)
...
plot_df_date <- HPAI_detections_date %>%
  select(date_det, all_of(names(colors))) %>%
  pivot_longer(
    cols = -date_det,
    names_to = "group",
    values_to = "count") %>%
  arrange(group, date_det) %>%
  group_by(group) %>%
  mutate(rolling_avg = slide_dbl(count, mean, .before = 3, .after = 3, .complete = FALSE)) %>%
  ungroup()

ggplot(plot_df_date, aes(x = date_det, y = rolling_avg, color = group)) +
  geom_line(size = 1.2) +
  scale_color_manual(values = colors) +
  labs(
    title = "Daily Rolling Average of HPAI Detections",
    x = "Date",
...
x_range <- range(c(HPAI_week$week, meta_week$week), na.rm = TRUE)
...
# fract of sequences vs detections 

meta_weekly <- meta %>%
  filter(!order_dg %in% c("nonhuman-mammal", "galliformes")) %>%
  mutate(week = floor_date(ymd(date), "week")) %>%
  count(week, name = "meta_count")

wild_weekly <- wildbirds %>%
  mutate(week = floor_date(ymd(date_det), "week")) %>%
  count(week, name = "wild_count")

combined <- full_join(meta_weekly, wild_weekly, by = "week") %>%
  arrange(week)

combined <- combined %>% 
  mutate(
    meta_count = replace_na(meta_count, 0),
    wild_count = replace_na(wild_count, 0),
    fraction = ifelse(wild_count == 0, NA, meta_count / wild_count))

ggplot(combined, aes(x = week, y = fraction)) +
  geom_line() +
  geom_point() +
  labs(
    x = "Week",
    y = "Fraction (sequence / wildbirds)",
    title = "Seq Records vs wildbird detcs") +
  theme_minimal()
