...
# compute OE ratio
compute_oe <- function(df, group_var, draw_var, n_draws = 1000) {
  observed_counts <- df %>%
      ...
      mutate(
        ...
        sim_value = map_chr(edge_props, function(ep) {
          ...
          if (length(values) == 0 || length(probs) == 0 ||
              any(is.na(probs)) || sum(probs, na.rm = TRUE) == 0) {
  ...
  sim_counts <- sim_results %>%
    filter(!is.na(sim_value)) %>%
    count(sim_id, value = sim_value, name = "sim_n") %>%
    rename(!!group_var := value)
  
  expected_summary <- sim_counts %>%
    group_by(.data[[group_var]]) %>%
    summarise(expected_mean = mean(sim_n), .groups = "drop")
  
  observed_counts %>%
    left_join(expected_summary, by = group_var) %>%
    mutate(oe_ratio = observed_n / expected_mean) %>%
...
bootstrap_oe <- function(df, group_var, draw_var, n_boot, n_draws) {
  ...
  with_progress({
    ...
    boot_results <- map_dfr(1:n_boot, function(b) {
      ...
      res <- compute_oe(boot_sample, group_var, draw_var, n_draws)
      res$boot_id <- b
      res
    })
  })
  ...
  boot_results %>%
    group_by(.data[[group_var]]) %>%
    summarise(
      oe_ratio_mean = mean(oe_ratio, na.rm = TRUE),
      oe_lower_boot = quantile(oe_ratio, 0.025, na.rm = TRUE),
      oe_upper_boot = quantile(oe_ratio, 0.975, na.rm = TRUE),
      n = sum(!is.na(oe_ratio)),
      p_upper = (sum(oe_ratio >= 1, na.rm = TRUE) + 1) / (n + 1),
      p_lower = (sum(oe_ratio <= 1, na.rm = TRUE) + 1) / (n + 1),
      p_boot  = pmin(1, 2 * pmin(p_upper, p_lower)),
      .groups = "drop") %>%
    mutate(p_label = formatC(p_boot, format = "e", digits = 2),
      signif  = ifelse(p_boot < 0.05, "*", ""))
}
...
build_segment_edge_props <- function(plot_df, edge_table, group_var) {
  segments_interval <- plot_df %>%
    ...
    mutate( interval_low  = tmrca_decimal_date - two_week_window / 2,
      interval_high = tmrca_decimal_date + two_week_window / 2)
  
  segments_interval %>%
    rowwise() %>%
    mutate( edges_in_interval = list( edge_table %>%
          filter(edge_decimal_date >= interval_low,
                 edge_decimal_date <= interval_high)),
      observed_n = ifelse( nrow(edges_in_interval) > 0 &&
          any(edges_in_interval[[group_var]] == .data[[group_var]]), 1, 0)) %>%
    ungroup() %>%
    ...
    filter(map_int(edge_props, nrow) > 0)
}

# plotting 
plot_panel_C <- function(df, group_var, fill_colors, title, xlab) {
  df %>% mutate(group = .data[[group_var]], group = factor(group, levels = group[order(oe_ratio_mean)])) %>%
    ggplot(aes(x = group, y = oe_ratio_mean, fill = group)) +
...
segment_edge_props <- build_segment_edge_props(plot_df, edge_table_ord, "ord")

final_orders <- bootstrap_oe(segment_edge_props, "ord", "ord", n_boot, n_draws)

p_orders_C <- plot_panel_C(final_orders, "ord", order_colors,
  "C) Orders (Bootstrap CI + Bootstrap Test)", "Host order")
...
final_flyway <- bootstrap_oe(segment_edge_props_flyway, "location", "location", n_boot, n_draws)

p_flyway_C <- plot_panel_C(
  final_flyway, "location", flyway_colors,
  "C) Flyways (Bootstrap CI + Bootstrap Test)", "Flyway")
