...
def calculate_rate_quantiles(log_data, mrsi):
    ...
    time_points = [mrsi - shift for shift in rate_shifts[:-1]]
...
def plot_reassortment_rates(ax, valid_times, medians, lower_95, upper_95, lower_50, upper_50, fromval, toval):
...
def plot_ne(ax_ne, valid_times, medians_ne, lower_95_ne, upper_95_ne, lower_50_ne, upper_50_ne, fromval, toval):
...
def create_clade_probability_plot(ax_clade_probs, ax_clade_heights, mrsi, path, fromval, toval):
    ...
    ax_clade_probs.set_ylabel('Reassortment Event Probability', fontsize=12)
    ax_clade_probs.tick_params(axis='both', labelsize=10)
    ax_clade_probs.tick_params(axis='x', rotation=45)
    ...
    ax_clade_heights.set_xlabel('Clade Height (years)', fontsize=12)
    ax_clade_heights.set_yticks(y_positions)
    ax_clade_heights.set_yticklabels(clade_names)
    ax_clade_heights.tick_params(axis='both', labelsize=10)
...
def save_figure(fig, approach):
    ...
    # save the figur into the /Users/nmueller/Documents/github/CoInfection-Material/Figures directory
    figname = 'H5N1_' + approach + '_combined.pdf'
    figname = os.path.join('/Users/nmueller/Documents/github/CoInfection-Material/Figures', figname)
...
def main():
    ...
    ax_clade_probs = fig.add_subplot(gs[0, 1], facecolor='w')
    ax_clade_heights = fig.add_subplot(gs[0, 2], facecolor='w')
    ax_probs = fig.add_subplot(gs[1, 1], facecolor='w')
    ax_ne = fig.add_subplot(gs[2, 1:2], facecolor='w')
    ...
    offset_y, stretch_y = plot_reassortment_rates(ax, valid_times, medians, lower_95, upper_95, lower_50, upper_50, start_time, toval)
    plot_ne(ax_ne, valid_times, medians_ne, lower_95_ne, upper_95_ne, lower_50_ne, upper_50_ne, start_time, toval)
    add_rate_axis(ax, time_points, offset_y, stretch_y, start_time, toval)
