...
file.remove(list.files("./xmls2", pattern="*.xml", full.names=TRUE))
...
no_samples = 500;
# get all fasta files in xml containing HA
fasta_files = list.files("./xmls", pattern="HA.fasta", full.names=TRUE)


segments = c("HA", "NA", "MP", "NS", "NP", "PB1", "PB2", "PA")
for (i in 1:length(fasta_files)) {
  fasta1 <- seqinr::read.fasta(file = fasta_files[i], seqtype = "DNA")
  isolates = names(fasta1)
  # randomly sample no_samples isolates
  isolates = sample(isolates, min(no_samples, length(isolates)))
  
  for (j in 1:length(segments)){
    new_fname = gsub("HA", segments[j], fasta_files[i])
    fasta2 <- seqinr::read.fasta(file = new_fname, seqtype = "DNA")
    # make fasta3 with only the isolates in isolates
    fasta3 = list()
    for (k in 1:length(isolates)){
      fasta3[[k]] = fasta2[[which(names(fasta2) == isolates[k])]]
    }
    # save fasta 3 into xmls2
    seqinr::write.fasta(sequences = fasta3, names = isolates, file = gsub("/xmls/", "/xmls2/", new_fname), open = "w")
  }
...
for (a in 1:length(virus)) {
  for (b in 1:length(year)){
    ...
    if (!file.exists(paste("./xmls2/", base, "_HA.fasta", sep=""))) {
      next
    }
    
    fasta1 <- seqinr::read.fasta(file = paste("./xmls2/", base, "_HA.fasta", sep=""), seqtype = "DNA")
    ...
    filename = paste(virus[[a]], "_", year[[b]] , ".skyline", sep="")
    # build an inference xml files
    f <- file(sprintf('xmls2/%s.rep0.xml',filename), 'w')
    ...
    while (length(line <- readLines(template, n = 1)) > 0) {
      ...
      #   
        ...
        seq1 = seqinr::read.fasta(file = paste("./xmls2/", base, "_HA.fasta", sep=""), seqtype = "DNA")
        seq2 = seqinr::read.fasta(file = paste("./xmls2/", base, "_NA.fasta", sep=""), seqtype = "DNA")
        seq3 = seqinr::read.fasta(file = paste("./xmls2/", base, "_MP.fasta", sep=""), seqtype = "DNA")
        seq4 = seqinr::read.fasta(file = paste("./xmls2/", base, "_NS.fasta", sep=""), seqtype = "DNA")
        seq5 = seqinr::read.fasta(file = paste("./xmls2/", base, "_NP.fasta", sep=""), seqtype = "DNA")
        seq6 = seqinr::read.fasta(file = paste("./xmls2/", base, "_PB1.fasta", sep=""), seqtype = "DNA")
        seq7 = seqinr::read.fasta(file = paste("./xmls2/", base, "_PB2.fasta", sep=""), seqtype = "DNA")
        seq8 = seqinr::read.fasta(file = paste("./xmls2/", base, "_PA.fasta", sep=""), seqtype = "DNA")
    ...
    # make a second xml where the .trees is replaced by .infected
    filename = paste(virus[[a]], "_", year[[b]] , ".skygrowth", sep="")
    f <- file(sprintf('xmls2/%s.rep0.xml', filename), 'w')
    # Open the template file
    template <- file('../H5N1NorthAmerica/inference_template_wgs_cr.xml', 'r')
    while (length(line <- readLines(template, n = 1)) > 0) {
...
# and .rep2.xml
for (file in list.files("./xmls2", pattern="*.rep0.xml", full.names=TRUE)) {
