...
file.remove(list.files("./xmlsLPAI/", pattern="*.xml", full.names=TRUE))

# get all fasta files in data
fasta_files = list.files("./data/LPAIHxNx_450/", pattern="*.fasta", full.names=TRUE)
# remove all fasta files that do not contain HA
fasta_files = fasta_files[grep("MP", fasta_files)]
...
for (fastafile in fasta_files){
  ...
  filename_base = paste(strsplit(basename(fastafile), "_")[[1]][1], strsplit(basename(fastafile), "_")[[1]][2], sep="_")
  
  # read in the log file to get the network height
  filename = paste(filename_base, ".dependent", sep="")
  # build an inference xml files
  f <- file(sprintf('xmlsLPAI/%s.rep0.xml',filename), 'w')
  # Open the template file
  template <- file('../H5N1NorthAmerica/inference_template_wgs_cr_6seg.xml', 'r')
  ...
  while (length(line <- readLines(template, n = 1)) > 0) {
    if (grepl('insert_name', line)) {
      ...
      writeLines(gsub('insert_seg1', paste(filename_base,"MP", sep="_"), line), f)
    }else if (grepl('insert_seg2', line)){
      writeLines(gsub('insert_seg2', paste(filename_base,"NS", sep="_"), line), f)
    }else if (grepl('insert_seg3', line)){
      writeLines(gsub('insert_seg3', paste(filename_base,"NP", sep="_"), line), f)
    }else if (grepl('insert_seg4', line)){
      writeLines(gsub('insert_seg4', paste(filename_base,"PB1", sep="_"), line), f)
    }else if (grepl('insert_seg5', line)){
      writeLines(gsub('insert_seg5', paste(filename_base,"PB2", sep="_"), line), f)
    }else if (grepl('insert_seg6', line)){
      writeLines(gsub('insert_seg6', paste(filename_base,"PA", sep="_"), line), f)
    ...
    #   writeLines(gsub('<!--isNeSkyline', '', line), f)  
      ...
      seq1 = seqinr::read.fasta(file = paste("./data/LPAIHxNx_450/",filename_base, "_HA.fasta", sep=""), seqtype = "DNA")
      seq2 = seqinr::read.fasta(file = paste("./data/LPAIHxNx_450/",filename_base, "_NA.fasta", sep=""), seqtype = "DNA")
      seq3 = seqinr::read.fasta(file = paste("./data/LPAIHxNx_450/",filename_base, "_MP.fasta", sep=""), seqtype = "DNA")
      seq4 = seqinr::read.fasta(file = paste("./data/LPAIHxNx_450/",filename_base, "_NS.fasta", sep=""), seqtype = "DNA")
      seq5 = seqinr::read.fasta(file = paste("./data/LPAIHxNx_450/",filename_base, "_NP.fasta", sep=""), seqtype = "DNA")
      seq6 = seqinr::read.fasta(file = paste("./data/LPAIHxNx_450/",filename_base, "_PB1.fasta", sep=""), seqtype = "DNA")
      seq7 = seqinr::read.fasta(file = paste("./data/LPAIHxNx_450/",filename_base, "_PB2.fasta", sep=""), seqtype = "DNA")
      seq8 = seqinr::read.fasta(file = paste("./data/LPAIHxNx_450/",filename_base, "_PA.fasta", sep=""), seqtype = "DNA")
      ...
      writeLines(gsub('insert_weights', paste(
  ...
  while (length(line <- readLines(template, n = 1)) > 0) {
    ...
    #   writeLines(gsub('<!--isinfectedSkyline', '', line), f)
      ...
      sequence_length_seg7 <- nchar(getSequence(seq7[[1]], as.string = TRUE))
      sequence_length_seg8 <- nchar(getSequence(seq8[[1]], as.string = TRUE))
      writeLines(gsub('insert_weights', paste(sequence_length_seg3, sequence_length_seg4,
                                              sequence_length_seg5, sequence_length_seg6,
...
for (fastafile in fasta_files){
  ...
  # get the filename as everything before the first _
  filename_base = strsplit(basename(fastafile), "\\.")[[1]][1]
  
  # read in the log file to get the network height
  filename = paste(filename_base, ".biceps", sep="")
  # build an inference xml files
  f <- file(sprintf('xmlsLPAI/%s.rep0.xml',filename), 'w')
  # Open the template file
  template <- file('../H5N1NorthAmerica/inference_BICEPS.xml', 'r')
  
  base=""
  while (length(line <- readLines(template, n = 1)) > 0) {
    if (grepl('insert_seg', line)){
      writeLines(gsub('insert_seg', filename_base, line), f)
     } else if (grepl('insert_heights', line)) {
      # write the height of each tip using heights seperated by a = and , between
      value=''
      for (i in seq(1, length(isolates))) {
...
files = list.files("xmlsLPAI", pattern="*.rep0.xml", full.names=TRUE)
for (file in files) {
